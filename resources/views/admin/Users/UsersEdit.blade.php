                ...
                <div class="card">
                    ...
                    <div class="card-body pt-0">
                        <form action="{{ route('Users.update', $Users->id) }}" method="post">
                            ...
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="floatinpassword" name="password"
                                    value="{{ $Users->UserRole->Role->role != 'Peserta Didik' ? 'Password Dapodik' : 'NISN Siswa' }}"
                                    onclick="this.value='';" autocomplete="off" />
                                <label for="floatinpassword">Password
                                </label>
                            </div>
                            <div class="form-floating mb-3">
                                <select class="form-select" id="floatingWaliRole" name="Role">
                                    @forelse ($Role as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $Users->UserRole->role_id == $item->id ? 'selected' : '' }}>
                                            {{ $item->role }}
                                        </option>
                                    @empty
