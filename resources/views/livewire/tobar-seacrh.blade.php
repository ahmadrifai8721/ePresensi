        ...
        <div class="dropdown-menu dropdown-menu-animated dropdown-xxl" style="overflow: scroll; max-height: 650px;"
            ...
            @if ($total >= 1)
                ...
                <div class="dropdown-header noti-title">
                    <h5 class="text-overflow mb-2">Ditemukan <span class="text-danger">{{ $total }}</span> data
                    </h5>
                </div>
            @endif
            @if ($siswa)
                ...
                @forelse ($pembelajaranData as $item)
                    <a href="{{ route('pembelajaran.show', $item->id) }}" class="dropdown-item notify-item">
                        ...
                        <span>
                            ...
                            {{ $item->kelas()->first() == null ? 'Kelas Telah di hapus' : $item->kelas()->first()->kelas }}<br>
                            {{ $item->guru->nama }}
                        </span>
                    </a>
            ...
            @if ($pkl)
                ...
                <!-- PKL-->
                <div class="dropdown-header noti-title">
                    <h6 class="text-overflow mb-2 text-uppercase">{{ $pkl }} Tempat PKL Di Temukan</h6>
                </div>
                @forelse ($pklData as $item)
                    <a href="{{ route('adminPKL.show', $item->id) }}" class="dropdown-item notify-item">
                        <i class="mdi mdi-google-classroom font-16 me-1"></i>
                        <span>
                            {{ $item->nama }} <br>
